...
fn test_watcher_moves_new_file() -> anyhow::Result<()> {
    ...
    config.rules = vec![create_test_rule("Images", vec!["jpg"], vec![])];
    ...
    assert!(expected_path.exists(), "Watcher should have moved the file to Images/");
...
fn test_watcher_batch_processing_and_conflicts() -> anyhow::Result<()> {
    ...
    config.source_dir = root.to_path_buf();
    config.rules = vec![
        create_test_rule("Docs", vec!["pdf", "txt"], vec![]),
        create_test_rule("Images", vec!["png"], vec![]),
    ];
    config.debounce_seconds = 1;
    ...
    assert!(root.join("Docs/report_1.pdf").exists(), "Conflict file should be renamed to report_1.pdf");
    assert!(root.join("Docs/note.txt").exists());
    assert!(root.join("Images/logo.png").exists());

    assert!(!subfolder.exists(), "The empty subfolder should have been cleaned up");
...
fn test_watcher_basic_flow_simple() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Docs", vec!["pdf"], vec![])];
    config.debounce_seconds = 1;

    let config_clone = config.clone();
    thread::spawn(move || { let _ = watch_mode(&config_clone, false); });
    thread::sleep(Duration::from_millis(500));
    ...
    assert!(root.join("Docs/test.pdf").exists(), "PDF file should be moved to Docs folder");
    assert!(!f.exists(), "Original file should be deleted after successful move");
    Ok(())
}
...
fn test_watcher_batch_and_conflict_resolution() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Images", vec!["png"], vec![])];
    config.debounce_seconds = 1;
    ...
    assert!(root.join("Images/logo.png").exists(), "Original logo file should still exist");
    assert!(root.join("Images/logo_1.png").exists(), "New file with same name should be renamed to logo_1.png to prevent data loss");
    Ok(())
}
...
fn test_watcher_unicode_and_special_names() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("เอกสาร", vec!["docx"], vec![])];
    config.debounce_seconds = 1;
    ...
    assert!(root.join("เอกสาร/รายงาน_สรุป #2024 🔥.docx").exists(), "Watcher should support Thai and emoji filenames correctly");
    Ok(())
}
...
fn test_watcher_should_not_move_ignored_files() -> anyhow::Result<()> {
    ...
    config.rules = vec![create_test_rule("Docs", vec!["txt"], vec![])];
    config.debounce_seconds = 1;
    ...
    assert!(!root.join("Docs/ignore_this.txt").exists(), "Ignored file should not appear in destination folder");
    Ok(())
}
#[test]
fn test_watcher_massive_burst_100_files() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Data", vec!["csv"], vec![])];
    config.debounce_seconds = 1;
    ...
    assert_eq!(count, 100, "System should be able to collect and move 100 files in a single scan");
    Ok(())
}
...
fn test_watcher_nested_recursive_organization() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Code", vec!["rs"], vec![])];
    config.debounce_seconds = 1;
    ...
    assert!(root.join("Code/main.rs").exists(), "File in deeply nested folder should be found and moved to destination folder");
    Ok(())
}
...
fn test_watcher_unknown_file_type_handling() -> anyhow::Result<()> {
    ...
    assert!(root.join("Etc/something.weird").exists(), "File that doesn't match any rule should be moved to Unknown (Etc) folder");
    Ok(())
}
...
fn test_watcher_empty_directory_cleanup_chain() -> anyhow::Result<()> {
    ...
    assert!(root.join("Docs/task.txt").exists(), "File must be moved out");
    assert!(!root.join("level1").exists(), "All empty source folders must be deleted (Cleanup Chain)");
    Ok(())
}
#[test]
fn test_watcher_massive_filename_length() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Overflow", vec!["bin"], vec![])];
    config.debounce_seconds = 1;
    ...
    assert!(root.join("Overflow").join(long_name).exists(), "File with abnormal length of 200+ characters should be moved without crashing the system");
    Ok(())
}
...
fn test_watcher_exclude_hidden_files() -> anyhow::Result<()> {
    ...
    config.rules = vec![create_test_rule("Protected", vec!["config"], vec![])];
    config.debounce_seconds = 1;
    ...
    assert!(hidden_file.exists(), "Hidden files starting with dot (.) must not be moved, even if their extension matches the rule");
    assert!(!root.join("Protected/.my_config.config").exists(), "Hidden files must not appear in the organized folder");
    Ok(())
}
