...
fn test_scanner_empty_dir() -> anyhow::Result<()> {
    ...
    // Path needs to be set properly for the config
...
fn test_scanner_mixed_rules_priority() -> anyhow::Result<()> {
    ...
    config.rules = vec![
        ...
        create_test_rule("Archives", vec!["pdf"], vec![]),
    ];
...
fn test_scanner_glob_patterns() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Invoices", vec![], vec!["invoice_*.pdf"])];
...
fn test_scanner_large_extension_list() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Images", extensions, vec![])];
...
fn test_scanner_overlapping_rules_glob_vs_ext() -> anyhow::Result<()> {
    ...
    config.rules = vec![
        create_test_rule("Special", vec![], vec!["special_*.pdf"]),
        create_test_rule("Docs", vec!["pdf"], vec![]),
    ];
...
fn test_scanner_rule_with_dots_in_extension() -> anyhow::Result<()> {
    ...
    let mut config = create_test_config();
    config.source_dir = root.to_path_buf();
    config.rules = vec![create_test_rule("Archives", vec!["tar.gz"], vec![])];
...
fn test_scanner_unicode_and_special_chars() -> anyhow::Result<()> {
    ...
    config.source_dir = root.to_path_buf();
    config.rules = vec![
        create_test_rule("Images", vec!["jpg", "png"], vec![]),
        create_test_rule("Docs", vec!["txt"], vec![]),
    ];
